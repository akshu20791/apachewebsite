pipeline {
    ...
    stages{
        ...
        stage('Build docker image'){
            steps{
                script{
                    sh 'docker build -t coshik/coshnewimg8july:v1 .'
                    sh 'docker images'
                }
            }
        }
          stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-pwd', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push coshik/coshnewimg8july:v1'
                }
            }
        }
            ...
            steps {
               script {
                   def dockerrm = 'sudo docker rm -f My-first-containe2211 || true'
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe2211 -p 8083:80 coshik/coshnewimg8july:v1'
                    sshagent(['sshkeypair']) {
